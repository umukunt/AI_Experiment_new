...
def merge_int16_arrays(left, right):
    ...
    """
    if isinstance(left, np.ndarray) and left.dtype == np.int16 and isinstance(right,
                                                                              np.ndarray) and right.dtype == np.int16:
...
class RealtimeAPI(RealtimeEventHandler):
    ...
    async def connect(self):
        ...
        self.ws = await websockets.connect(
            f"{self.url}/openai/realtime?api-version={self.api_version}&deployment={self.azure_deployment}&api-key={self.api_key}")
...
class RealtimeConversation:
    ...
    EventProcessors = {
        ...
        'conversation.item.input_audio_transcription.completed': lambda self,
                                                                        event: self._process_input_audio_transcription_completed(
            event),
        'input_audio_buffer.speech_started': lambda self, event: self._process_speech_started(event),
        'input_audio_buffer.speech_stopped': lambda self, event, input_audio_buffer: self._process_speech_stopped(event,
                                                                                                                  input_audio_buffer),
